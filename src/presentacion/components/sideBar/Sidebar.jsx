...
import { HOME_ROUTES } from "./SidebarItem";
...
export default function Sidebar({ navItems = [], footerLabel = "Usuario" }) {
  ...
  useEffect(() => {
    ...
    if (!localStorage.getItem("rolActivo")) {
      localStorage.setItem("rolActivo", usuario.roles[0]);
    }
  }, [usuario]);
  ...
  return (
    <aside className="w-[300px] h-screen sticky top-0 bg-[#e8192c] flex flex-col shadow-[4px_0_16px_rgba(232,25,44,0.18)] flex-shrink-0">
      ...
      <div className="flex items-center justify-center min-h-[80px] px-3 py-4 border-b-[3px] border-[#e8192c]">
        <img src={logo} alt="SISPRAC" className=" h-20" />
      </div>
              ...
              <>
                ...
                {usuario.roles.map((rol) => (
                  <button
                    ...
                    className={`w-full flex items-center px-4 py-3 text-left hover:bg-gray-100 ${rol === rolActivo
